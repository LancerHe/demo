    ...
    czUI.czTab.prototype = {
		...
		init: function() {
			...
			if ( location.hash ) {
				...
				$tag = this.$element.find('.' + this.options.tabClass + "[tag='"+hash+"']");
				if ( $tag.length > 0 ) {
					this.$tab.each(function(i) {
						try {
							if ( $(this).attr('tag') == hash ) {
								throw i
							}
						} catch(e) {
							self.$element.find('.' + self.options.activeClass ).removeClass( self.options.activeClass );
							self.$element.find('.' + self.options.panelClass + ':visible').hide();
                    		self.$tab.eq(i).addClass( self.options.activeClass );
							self.$panel.eq(i).show();
						}
					});
				}
			}
        ...
        _setTab: function() {
            ...
            switch (this.options.eventType) {
                case 'hover': var eventType = 'mouseenter';break;
				case 'click': var eventType = 'click';break;
                default: var eventType = 'click';break;
            }
        ...
        showPanel: function(index) {
            ...
            switch (this.options.displayType) {
                case 'fade':
                    this.$element.find('.' + this.options.panelClass + ':visible').fadeOut( self.options.displaySpeed, function() {
                        self._callback('hide');
                        self.$panel.eq(index).fadeIn(function() {
                            self._callback('show');
                        });
                    });
                    break;
                ...
                case 'slide':
                    this.$element.find('.' + this.options.panelClass + ':visible').slideUp( self.options.displaySpeed, function() {
                        self._callback('hide');
                        self.$panel.eq(index).slideDown(function() {
                            self._callback('show');
                        });
                    });
                    break;
                ...
                default: //click
                    this.$element.find('.' + this.options.panelClass + ':visible').hide();
